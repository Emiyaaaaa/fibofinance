...
const LineChart = React.forwardRef<HTMLDivElement, LineChartProps>((props, ref) => {
  ...
  const [activePayload, setActivePayload] = React.useState<PayloadItem | undefined>(undefined);
  ...
  function onDotClick(itemData: any, event: React.MouseEvent) {
    ...
    if (
      ...
      setActivePayload(itemData.payload);
      onValueChange?.({
        eventType: "dot",
          ...
          <Tooltip
            ...
            content={({ active, payload, label }) => {
              ...
              return showTooltip && active ? (
                CustomTooltip ? (
                  <CustomTooltip active={active} activePayload={activePayload} label={label} payload={cleanPayload} />
                ) : (
                  <ChartTooltip active={active} label={label} payload={cleanPayload} valueFormatter={valueFormatter} />
                )
              ) : null;
            }}
                  ...
                  return (
                    <Dot
                      ...
                      className={cx(
                        ...
                        hasOnValueChange ? "cursor-pointer" : "",
                        getColorClassName(categoryColors.get(dataKey) as AvailableChartColorsKeys, "fill"),
                        activeDot && !isActive && showDots ? "opacity-60" : "",
                      )}
                      cx={cxCoord}
                      cy={cyCoord}
                      fill=""
